...
<body>
    ...
    <h3 class="title">Laporan Pendampingan</h3>
    ...
    <table>
        ...
        <tbody>
            ...
            @foreach ($assistance as $index => $assistances)
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td>{{ $assistances->assistance_name }}</td>
                    <td>{{ $assistances->training->training_name ?? '-' }}</td>
                    <td>{{ Carbon::parse($assistances->start_date)->translatedFormat('d F Y') }}</td>
                    <td>{{ Carbon::parse($assistances->end_date)->translatedFormat('d F Y') }}</td>
                    <td>{{ Carbon::parse($assistances->deadline_date)->translatedFormat('d F Y') }}</td>
                    <td>{{ strtoupper($assistances->status) }}</td>
                    <td>{{ $assistances->location }}</td>
                    <td>{{ $assistances->target_audience }} Orang</td>
                    <td>{{ $assistances->assistance_users_count }} Orang</td>
                </tr>
            @endforeach
        </tbody>
    </table>
